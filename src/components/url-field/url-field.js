...
import css from "./url-field.css?inline";
import sharedCss from "../../shared/shared.css?inline";
import html from "./url-field.html?raw";
import { URL_FIELD } from "../../shared/form-field-properties.js";

export default class CucumberUrlField extends FormElement {
	constructor() {
		super().render(html, css, sharedCss);
	}

	/** @type {string} */
	static __localName = 'cc-url-field';
	...
	/**
	 * Update prefix text, e.g.: https://giuthub.com/
	 */
	onPrefixTextChange(text) {
		this.shadowRoot.querySelector('[part="prefix"]').textContent = text;
	}
	...
	attributeChangedCallback(attr, oldValue, newValue) {
		super.attributeChangedCallback(attr, oldValue, newValue, URL_FIELD);
	}
}

if (!customElements.get(CucumberUrlField.__localName)) {
	customElements.define(CucumberUrlField.__localName, CucumberUrlField);
}

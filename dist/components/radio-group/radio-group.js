import{F as s}from"../assets/form-element-1a56e9b6.js";import"../assets/base-element-f411db3f.js";customElements.get("cc-radio-group")||customElements.define("cc-radio-group",class extends s{set checked(e){this.input.checked=e,this.input.ariaChecked=e?"true":"false"}set disabled(e){this.setAttribute("aria-disabled",e?"true":"false"),e?this.defaultSlot.assignedElements().forEach(t=>t.setAttribute("disabled","")):this.defaultSlot.assignedElements().forEach(t=>t.removeAttribute("disabled"))}get value(){const e=this.defaultSlot.assignedElements().find(t=>t.hasAttribute("checked"));return e?e.value:null}set label(e){this.shadowRoot.querySelector('slot[name="label"]').textContent=e}connectedCallback(){super.render(`<fieldset part="fieldset" role="radiogroup">
	<label part="label">
